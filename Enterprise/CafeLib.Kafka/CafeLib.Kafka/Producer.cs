        ...
        private async Task BatchSendAsync()
        {
            ...
            while (_asyncCollection.IsCompleted == false || _asyncCollection.Count > 0)
            {
                ...
                try
                {
                    ...
                    {
                        batch ??= new List<TopicMessage>(_asyncCollection.Count);
                    ...
                    var _ = sendTask.ContinueWith(result =>
                    {
                        if (result.IsFaulted)
                        {
            ...
            foreach (var ackLevelBatch in messages.GroupBy(batch => new { batch.Acks, batch.Timeout }))
            {
                ...
                foreach (var group in messageByRouter)
                {
                    ...
                    var request = new ProduceRequest
                    {
                        ...
                        TimeoutMilliseconds = (int)ackLevelBatch.Key.Timeout.TotalMilliseconds,
                    ...
                    await brokerSendTask.Task.ContinueWith(t => { _semaphoreMaximumAsync.Release(); }, cancellationToken);
