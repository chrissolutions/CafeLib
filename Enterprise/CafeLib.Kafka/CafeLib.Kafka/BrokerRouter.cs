...
namespace CafeLib.Kafka
{
    ...
    public class BrokerRouter : IBrokerRouter
    {
        ...
        public BrokerRoute SelectBrokerRoute(string topic, int partitionId)
        {
            ...
            if (partition == null) throw new InvalidPartitionException($"The topic:{topic} does not have a partitionId:{partitionId} defined.");
        ...
        private BrokerRoute GetCachedRoute(string topic, Partition partition)
        {
            ...
            throw new LeaderNotFoundException($"Lead broker cannot be found for parition: {partition.PartitionId}, leader: {partition.LeaderId}");
        }

        private BrokerRoute TryGetRouteFromCache(string topic, Partition partition)
        {
            if (_brokerConnectionIndex.TryGetValue(partition.LeaderId, out var conn))
            {
