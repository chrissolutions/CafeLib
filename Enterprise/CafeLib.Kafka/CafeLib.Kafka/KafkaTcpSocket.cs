...
using CafeLib.Kafka.Statistics;
...
namespace CafeLib.Kafka
{
    ...
    public class KafkaTcpSocket : IKafkaTcpSocket
    {
        ...
        /// </summary>
        public KafkaEndpoint Endpoint => _endpoint;
        ...
        private async Task ProcessReadTaskAsync(NetworkStream netStream, SocketPayloadReadTask readTask)
        {
            using (readTask)
            {
                try
                {
                    ...
                    while (bytesReceived < readSize)
                    {
                        ...
                        if (bytesReceived <= 0)
                        {
                            ...
                            {
                                _client = null;
                                throw new ServerDisconnectedException($"Lost connection to server: {_endpoint}");
                            }
                        }
                ...
                catch (Exception ex)
                {
                    ...
                    {
                        var exception = new ServerDisconnectedException($"Lost connection to server: {_endpoint}");
                        readTask.Tcp.TrySetException(exception);
                        throw exception;
                    }
