...
namespace CafeLib.Kafka
{
    ...
    public class KafkaConnection : IKafkaConnection
    {
        ...
        private readonly TimeSpan _responseTimeoutMilliseconds;
        ...
        public KafkaConnection(IKafkaTcpSocket client, TimeSpan? responseTimeoutMs = null, IKafkaLog log = null)
        {
            ...
            _responseTimeoutMilliseconds = responseTimeoutMs ?? TimeSpan.FromMilliseconds(DefaultResponseTimeoutMs);
            ...
            if (request.ExpectResponse)
            {
                ...
                try
                {
                    ...
                    await _client.WriteAsync(request.Encode())
                        ...
                        .ContinueWith(t => asyncRequest.MarkRequestAsSent(t.Exception, _responseTimeoutMilliseconds, TriggerMessageTimeout))
        ...
        private void CorrelatePayloadToRequest(byte[] payload)
        {
            ...
            if (_requestIndex.TryRemove(correlationId, out var asyncRequest))
            {
        ...
        private void TriggerMessageTimeout(AsyncRequestItem asyncRequestItem)
        {
            ...
            _requestIndex.TryRemove(asyncRequestItem.CorrelationId, out _); //just remove it from the index
            ...
            {
                asyncRequestItem.ReceiveTask.TrySetException(new ResponseTimeoutException($"Timeout Expired. Client failed to receive a response from server after waiting {_responseTimeoutMilliseconds}ms."));
            }
        }
