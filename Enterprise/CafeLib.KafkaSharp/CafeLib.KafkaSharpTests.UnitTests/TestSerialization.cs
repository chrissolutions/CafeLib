...
using Xunit;
using Record = CafeLib.KafkaSharp.Protocol.Record;

namespace CafeLib.KafkaSharpTests.UnitTests
{
    public class TestSerialization
    {
        private const string TheKey = "The Key Opens the Door";
        private const string TheValue = "The quick brown fox jumps over the zealy göd.";
        private const string TheClientId = "ClientId";
        ...
        private static void CompareArrays<T>(IReadOnlyList<T> expected, IReadOnlyList<T> compared, int offset)
        {
            for (int i = 0; i < expected.Count; ++i)
            {
                Assert.Equal(expected[i], compared[i + offset]);
            }
        }

        private static void CompareBuffers(IReadOnlyList<byte> expected, MemoryStream compared)
        {
            CompareArrays(expected, compared.GetBuffer(), (int) compared.Position);
            compared.Position += expected.Count;
        }
        ...
        public void TestSerializeOneMessageWithPreserializedKeyValue()
        {
            ...
            Assert.NotNull(message.SerializedKeyValue);
            TestSerializeOneMessageCommon(message);
        ...
        internal class SimpleSerializable : IMemorySerializable
        {
        ...
        internal class SimpleSizedSerializable : SimpleSerializable, ISizedMemorySerializable
        {
        ...
        public void TestSerializeOneMessage_V1()
        {
            ...
            Assert.Equal(GetExpectedMessageSize(Key.Length, Value.Length, MessageVersion.V1), serialized.Length);
            Assert.Equal(1, serialized.GetBuffer()[4]); // magic byte is 1
            Assert.Equal(0, serialized.GetBuffer()[5]); // attributes is 0
            serialized.Position = 6;
            Assert.Equal(42, BigEndianConverter.ReadInt64(serialized));
            Assert.Equal(Key.Length, BigEndianConverter.ReadInt32(serialized));
            CompareBuffers(Key, serialized);
            Assert.Equal(Value.Length, BigEndianConverter.ReadInt32(serialized));
            CompareBuffers(Value, serialized);
        }

        private static void TestSerializeOneMessageCommon(Message message)
        {
            using (var serialized = new ReusableMemoryStream(null))
            {
                message.Serialize(serialized, CompressionCodec.None, new Tuple<ISerializer, ISerializer>(null, null), MessageVersion.V0);
                Assert.Equal(GetExpectedMessageSize(Key.Length, Value.Length, MessageVersion.V0), serialized.Length);
                Assert.Equal(0, serialized.GetBuffer()[4]); // magic byte is 0
                Assert.Equal(0, serialized.GetBuffer()[5]); // attributes is 0
                serialized.Position = 6;
                Assert.Equal(Key.Length, BigEndianConverter.ReadInt32(serialized));
                CompareBuffers(Key, serialized);
            ...
            using (var serialized = new ReusableMemoryStream(null))
            {
                ...
                Assert.Equal(NullKeyMessageSize, serialized.Length);
                Assert.Equal(0, serialized.GetBuffer()[4]); // magic byte is 0
                Assert.Equal(0, serialized.GetBuffer()[5]); // attributes is 0
                serialized.Position = 6;
                Assert.Equal(-1, BigEndianConverter.ReadInt32(serialized));
                Assert.Equal(Value.Length, BigEndianConverter.ReadInt32(serialized));
                CompareBuffers(Value, serialized);
            }
        }
        ...
        public void TestSerializeOneMessageCodec()
        {
            ...
            using (var serialized = new ReusableMemoryStream(null))
            {
                ...
                Assert.Equal(0, serialized.GetBuffer()[4]); // magic byte is 0
                Assert.Equal(2, serialized.GetBuffer()[5]); // attributes is 2
            }
            ...
            using (var serialized = new ReusableMemoryStream(null))
            {
                ...
                Assert.Equal(0, serialized.GetBuffer()[4]); // magic byte is 0
                Assert.Equal(1, serialized.GetBuffer()[5]); // attributes is 1
            }
        }

        [Theory]
        [InlineData(MessageVersion.V0)]
        [InlineData(MessageVersion.V1)]
        internal void TestSerializeOneEmptyByteMessage(MessageVersion messageversion)
        {
        ...
        [InlineData(MessageVersion.V0)]
        [InlineData(MessageVersion.V1)]
        internal void TestSerializeOneEmptySerializableMessage(MessageVersion messageversion)
        {
        ...
        [InlineData(MessageVersion.V0)]
        [InlineData(MessageVersion.V1)]
        internal void TestSerializeOneEmptyMessageWithPreserializedKeyValue(MessageVersion messageVersion)
        {
            var message = new Message { Key = new byte[0], Value = new byte[0] };
            message.SerializeKeyValue(new ReusableMemoryStream(null), new Tuple<ISerializer, ISerializer>(null, null), Compatibility.V0_8_2);
            Assert.Null(message.Value);
        ...
        private static void TestSerializeOneEmptyMessageCommon(Message message, MessageVersion messageVersion)
        {
            ...
            Assert.Equal(GetExpectedMessageSize(0, 0, messageVersion), serialized.Length);
            serialized.Position = 6;
            if (messageVersion == MessageVersion.V1)
            {
                Assert.Equal(message.TimeStamp, BigEndianConverter.ReadInt64(serialized));
            }
            Assert.Equal(0, BigEndianConverter.ReadInt32(serialized));
            Assert.Equal(0, BigEndianConverter.ReadInt32(serialized));
        }
        ...
        public void TestSerializeMessageWithInvalidPreserializedKeyValue()
        {
            ...
            Assert.NotNull(message.SerializedKeyValue);
            Assert.Equal(0, message.SerializedKeyValue.Length);

            // Verify that serialization is able to process the message anyway
            using (var serialized = new ReusableMemoryStream(null, logger))
            {
                message.Serialize(serialized, CompressionCodec.None, new Tuple<ISerializer, ISerializer>(null, null), MessageVersion.V0);
                Assert.Equal(GetExpectedMessageSize(0, 0, MessageVersion.V0), serialized.Length);
                Assert.Equal(0, serialized.GetBuffer()[4]); // magic byte is 0
                Assert.Equal(0, serialized.GetBuffer()[5]); // attributes is 0
                serialized.Position = 6;
        ...
        public void TestSerializeMessageSet()
        {
            ...
            Assert.Equal(42, BigEndianConverter.ReadInt32(serialized)); // Partition
            Assert.Equal(serialized.Length - 4 - 4, BigEndianConverter.ReadInt32(serialized)); // MessageSet size
            Assert.Equal(0, BigEndianConverter.ReadInt64(serialized)); // First message offset
            int firstMsgSize = BigEndianConverter.ReadInt32(serialized);
            serialized.Position += firstMsgSize;
            Assert.Equal(1, BigEndianConverter.ReadInt64(serialized)); // Second message offset
            int secondMsgSize = BigEndianConverter.ReadInt32(serialized);
            serialized.Position += secondMsgSize;
            Assert.Equal(serialized.Length, serialized.Position);
        }

        [Theory]
        [Category("failsOnMono")] //because Snappy uses native code
        [InlineData(CompressionCodec.Gzip, 1)]
        [InlineData(CompressionCodec.Snappy, 2)]
        [InlineData(CompressionCodec.Lz4, 3)]
        public void TestSerializeMessageSetCompressed(CompressionCodec codec, byte attr)
        {
            ...
            Assert.Equal(42, BigEndianConverter.ReadInt32(serialized)); // Partition
            Assert.Equal(serialized.Length - 4 - 4, BigEndianConverter.ReadInt32(serialized)); // MessageSet size
            Assert.Equal(0, BigEndianConverter.ReadInt64(serialized)); // Wrapper message offset
            ...
            Assert.Equal(attr, serialized.ReadByte()); // attributes of the wrapper msg
            Assert.Equal(-1, BigEndianConverter.ReadInt32(serialized)); // No key => size = -1
            Assert.Equal(serialized.Length - valuePos, BigEndianConverter.ReadInt32(serialized)); // size of compressed payload
        }

        [Theory]
        [Category("failsOnMono")] //because Snappy uses native code
        [InlineData(CompressionCodec.None)]
        ...
        public void TestDeserializeMessageSet_V0(CompressionCodec codec)
        {
            ...
            {
                Assert.Equal(o, msg.Offset);
                Assert.Equal(Key, msg.Message.Key as byte[]);
                Assert.Equal(Value, msg.Message.Value as byte[]);
                Assert.Equal(0, msg.Message.TimeStamp);
                ++o;
            }
        }
        ...
        public void TestDeserializeMessageSet_v1uncompressed()
        {
            ...
            foreach (var msg in deserialized)
            {
                ...
                Assert.Equal(Key, msg.Message.Key as byte[]);
                Assert.Equal(Value, msg.Message.Value as byte[]);
                Assert.Equal(set.Messages.ElementAt(o).TimeStamp, msg.Message.TimeStamp);
                ++o;
            }
        }
        ...
        public void TestDeserializeMessageSet_v1compressed(CompressionCodec codec)
        {
            using (var serialized = Pool.Reserve())
            {
                ...
                const long lastOffset = 87;
                BigEndianConverter.Write(serialized, lastOffset);
                ...
                foreach (var msg in deserialized)
                {
                    Assert.AreEqual(lastOffset + 1 - deserialized.Count + o, msg.Offset);
        ...
        {
            Assert.Equal(expected.Partition, BigEndianConverter.ReadInt32(stream)); //, "partition");
            Assert.Equal(stream.Length - 2 * sizeof(int), BigEndianConverter.ReadInt32(stream)); //, "size");
            Assert.Equal(0, BigEndianConverter.ReadInt64(stream)); //, "baseOffset");
            Assert.Equal(stream.Length - (3 * sizeof(int) + sizeof(long)), BigEndianConverter.ReadInt32(stream)); //, "batchLength");
            Assert.Equal(-1, BigEndianConverter.ReadInt32(stream)); //, "partitionLeaderEpoch");
            Assert.Equal(2, stream.ReadByte()); //, "magic");
            ...
            Assert.Equal(crcComputed, crcRead); //, "crc");

            short attributes = BigEndianConverter.ReadInt16(stream);
            Assert.Equal(0, attributes); //, "attributes");

            Message[] messages = expected.Messages.ToArray();

            Assert.Equal(messages.Length - 1, BigEndianConverter.ReadInt32(stream)); //, "lastOffsetDelta");
            Assert.Equal(0, BigEndianConverter.ReadInt64(stream)); //, "firstTimestamp");
            Assert.Equal(0, BigEndianConverter.ReadInt64(stream)); //, "maxTimestamp");
            Assert.Equal(-1, BigEndianConverter.ReadInt64(stream)); //, "producerId");
            Assert.Equal(-1, BigEndianConverter.ReadInt16(stream)); //, "producerEpoch");
            Assert.Equal(-1, BigEndianConverter.ReadInt32(stream)); //, "baseSequence");
            Assert.Equal(messages.Length, BigEndianConverter.ReadInt32(stream)); //, "recordsCount");
            ...
            foreach (var message in messages)
            {
                ...
                Assert.Equal(expectedKey.Length + expectedVal.Length + 6 * sizeof(byte), VarIntConverter.ReadAsInt64(stream)); //, "length");
                Assert.Equal(0, stream.ReadByte()); //, "attributes");
                Assert.Equal(0, VarIntConverter.ReadAsInt64(stream)); //, "timestampDelta");
                Assert.Equal(offset, VarIntConverter.ReadAsInt64(stream)); //, "offsetDelta");

                Assert.Equal(expectedKey.Length, VarIntConverter.ReadAsInt64(stream)); //, "keyLength");
                var key = new byte[expectedKey.Length];
                stream.Read(key, 0, key.Length);
                Assert.Equal(key, expectedKey); //, "key");

                Assert.Equal(expectedVal.Length, VarIntConverter.ReadAsInt64(stream)); //, "valueLength");
                var value = new byte[expectedVal.Length];
                stream.Read(value, 0, value.Length);
                Assert.Equal(value, expectedVal); //, "value");

                Assert.Equal(message.Headers?.Count ?? 0, VarIntConverter.ReadAsInt64(stream)); //, "headersCount");

                offset += 1;
            }
        }

        private static void PreserializeMessage(Message[] messages, Compatibility compat)
        {
