...
namespace CafeLib.KafkaSharp.Batching
{
    ...
    internal abstract class Accumulator<TData>  : IDisposable
    {
    ...
    internal interface IBatchByTopic<out TData> : IEnumerable<IGrouping<string, TData>>, IDisposable
    {
        int Count { get; }
    }
    ...
    /// </summary>
    /// <typeparam name="TData"></typeparam>
    internal class BatchByTopic<TData> : IBatchByTopic<TData>
    {
        private static readonly Pool<BatchByTopic<TData>> Pool = new Pool<BatchByTopic<TData>>(
        ...
        {
            if (!_batch.TryGetValue(topic, out var grouping))
            {
    ...
    /// </summary>
    /// <typeparam name="TData"></typeparam>
    internal class AccumulatorByTopic<TData> : Accumulator<TData>
    {
    ...
    internal class AccumulatorByNodeByTopic<TData> : Accumulator<Tuple<INode, TData>>
    {
        private readonly Func<TData, string> _topicFromData;
        ...
        {
            var (node, item) = data;
            if (!_currentBatches.TryGetValue(node, out var batch))
            {
                batch = BatchByTopic<TData>.New();
                _currentBatches[node] = batch;
            }
            batch.Add(_topicFromData(item), item);
        }
    }
    ...
    internal interface IBatchByTopicByPartition<out TData> : IEnumerable<IGrouping<string, IGrouping<int, TData>>>, IDisposable
    {
        int Count { get; }
    }

    internal class BatchByTopicByPartition<TData> : IBatchByTopicByPartition<TData>
    {
        private static readonly Pool<BatchByTopicByPartition<TData>> Pool = new Pool<BatchByTopicByPartition<TData>>(
            512, // Hard coded security to avoid memory explosion if you're doing someting wrong
    ...
    internal class AccumulatorByTopicByPartition<TData> : Accumulator<TData>
    {
        private readonly Func<TData, string> _topicFromData;
    ...
    internal class AccumulatorByNodeByTopicByPartition<TData> : Accumulator<Tuple<INode, TData>>
    {
        private readonly Func<TData, string> _topicFromData;
        private readonly Func<TData, int> _partitionFromData;
        ...
        protected override void Accumulate(Tuple<INode, TData> data)
        {
            if (!_currentBatches.TryGetValue(data.Item1, out var batch))
            {
