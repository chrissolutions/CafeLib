...
namespace CafeLib.Bitcoin.Shared.Crypto
{
    public static partial class Hashes
    {
        ...
        public static void Hash256(this ReadOnlyByteSpan data, ByteSpan hash)
        {
            var x = new UInt256();
            using var sha = new SHA256Managed();
            TransformFinalBlock(sha, data, x.Bytes);
            TransformFinalBlock(sha, x.Bytes, hash);
        }
