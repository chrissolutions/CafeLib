...
namespace CafeLib.Cryptography.Secp256K1.Recovery
{
#if SECP256K1_LIB
	public
#endif
	...
	{
		public static bool TryRecover(Context ctx, SecpRecoverableECDSASignature recoverableSig, ReadOnlySpan<byte> msg32, [MaybeNullWhen(false)] out Secp256K1.ECPubKey pubkey)
		{
			...
			{
				pubkey = new Secp256K1.ECPubKey(q, ctx);
				return true;
			}
			else
			{
		...
		static bool secp256k1_ecdsa_sig_recover(ECMultContext ctx, in Scalar sigr, in Scalar sigs, out GE pubkey, in Scalar message, int recid)
		{
			...
			if ((recid & 2) != 0)
			{
				if (fx.CompareToVariable(Secp256K1.ECPubKey.order_as_fe) >= 0)
				{
					pubkey = default;
					return false;
				}
				fx += Secp256K1.ECPubKey.order_as_fe;
			}
