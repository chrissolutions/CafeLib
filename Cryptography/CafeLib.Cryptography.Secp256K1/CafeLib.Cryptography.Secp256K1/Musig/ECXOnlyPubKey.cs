...
namespace CafeLib.Cryptography.Secp256K1.Musig
{
	...
	partial class ECXOnlyPubKey
	{
		...
		static void secp256k1_musig_compute_ell(Span<byte> ell32, Secp256K1.ECXOnlyPubKey[] pk)
		{
		...
		{
			using SHA256 sha = new SHA256();
			sha.InitializeTagged(Secp256K1.ECXOnlyPubKey.TAG_BIP0340Challenge);
 ...
 * second_pk_x are normalized. */
		...
		public static Secp256K1.ECXOnlyPubKey MusigCombine(Secp256K1.ECXOnlyPubKey[] pubkeys)
		{
			return MusigCombine(pubkeys, null);
		}

		internal static Secp256K1.ECXOnlyPubKey MusigCombine(Secp256K1.ECXOnlyPubKey[] pubkeys, MusigContext? preSession)
		{
			if (pubkeys == null)
			...
			var combined_pk = new Secp256K1.ECXOnlyPubKey(pkp, ctx);
